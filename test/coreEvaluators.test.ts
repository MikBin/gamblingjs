...
class TestBase extends BaseEvaluator {
  ...
  public v(h: any, n: number) {
    /* @ts-ignore */ return this.validateHand(h, n);
  }
  public nrm(h: number[]) {
    /* @ts-ignore */ return this.normalizeHand(h);
  }
}
...
describe('HighEvaluator error path', () => {
  ...
  it('throws for unsupported hand size', () => {
    expect(() => he.evaluate([0, 1, 2, 3, 4, 5])).toThrow(
      'HighEvaluator only supports 5-card and 7-card hands',
    );
  });
});
...
describe('LowAto5Evaluator coverage', () => {
  ...
  it('throws verbose not implemented', () => {
    expect(() => ev.evaluateVerbose([0, 1, 2, 3, 4, 5, 6])).toThrow(
      'Verbose evaluation not implemented for A-5 low',
    );
  });
  it('throws for wrong length', () => {
    // 6 cards
    expect(() => ev.evaluate([0, 1, 2, 3, 4, 5])).toThrow(
      'LowAto5Evaluator only supports 5-card and 7-card hands',
    );
  });
...
describe('Low8Evaluator coverage', () => {
  ...
  it('throws for wrong length', () => {
    expect(() => ev.evaluate([0, 1, 2, 3, 4, 5])).toThrow(
      'Low8Evaluator only supports 5-card and 7-card hands',
    );
  });
...
describe('Low9Evaluator coverage', () => {
  ...
  it('throws for wrong length', () => {
    expect(() => ev.evaluate([0, 1, 2, 3, 4, 5])).toThrow(
      'Low9Evaluator only supports 5-card and 7-card hands',
    );
  });
