// 6-max FIXED-LIMIT Texas Hold'em bot benchmark. Same matrix as bench-bots.ts
// (all bots except always-fold), each config vs a fixed smart-balanced field of
// 5, candidate at seat 0. bb/100 is normalized in BIG BLINDS (bb=2) so it stays
// comparable across the NL/PL/FL benches.
//
//   npx tsx tools/bench-fl-6max.ts [hands]
...
const PRESET = fixedLimitHoldem({ sb: 1, bb: 2, smallBet: 2, bigBet: 4, maxRaises: 4, stack: STACK });
...
console.log(`=== 6-MAX FIXED-LIMIT Hold'em (smallBet 2 / bigBet 4 / maxRaises 4, ${STACK}bb) — each config vs 5 smart-balanced ===`);
