...
const gameName = (gt: string) =>
  gt === 'omaha-hi' ? 'Omaha Hi'
    : gt === 'omaha-hi-lo' ? 'Omaha Hi/Lo'
    : gt === '7card-stud' ? '7-Card Stud'
    : gt === 'stud-hi-lo' ? 'Stud Hi/Lo'
    : gt === 'razz' ? 'Razz'
    : 'Texas Hold\'em';
...
export const formatTable = (result: SimulationResult): string => {
  ...
  const title = gameName(result.gameType);
