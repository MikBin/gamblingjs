...
import type { handCategoryDistribution } from '../../../src/interfaces';
...
export const useMonteCarlo = (options: MonteCarloOptions = {}) => {
  const {
    defaultRuns = 10000,
    maxRuns = 100000,
    minRuns = 1000
  ...
  const lastResult = ref<handCategoryDistribution | null>(null);
  const error = ref<string | null>(null);

  const isValidRunCount = (runs: number): boolean => {
    return runs >= minRuns && runs <= maxRuns;
  };

  const runSimulation = async (
    partialHand: number[],
    runs: number = totalRuns.value
  ): Promise<handCategoryDistribution | null> => {
  ...
  const setTotalRuns = (runs: number) => {
    if (isValidRunCount(runs)) {
      totalRuns.value = runs;
    }
  };
  ...
  return {
    ...
    // Computed
    isValidRunCount: computed(() => (runs: number) => isValidRunCount(runs)),
