...
export interface PokerEvaluatorOptions {
  gameVariant?: 'texas-holdem' | 'omaha' | 'seven-card-stud';
  evaluationType?: 'high' | 'low8' | 'low9' | 'Ato5' | 'Ato6' | '2to7';
}
...
export const usePokerEvaluator = (options: PokerEvaluatorOptions = {}) => {
  ...
  const evaluateHand = async (cards: number[]): Promise<verboseHandInfo | null> => {
    ...
    try {
      ...
      let verboseResult: verboseHandInfo | null = null;
      if (cards.length === 7 && typeof FIVE_CARD_POKER_EVAL.HandRankVerbose[7]?.high === 'function') {
        try {
          verboseResult = FIVE_CARD_POKER_EVAL.HandRankVerbose[7].high(
      ...
      const result: verboseHandInfo = verboseResult || {
        hand: cards,
        faces: '',
        handGroup: '',
        winningCards: [],
